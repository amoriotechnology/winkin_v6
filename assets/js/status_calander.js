...
window.onload = function () {
  ...
  monthArray.forEach((month, index) => {
    ...
    if((currentMonth == index) || (setmonth == index)) {
  ...
  leftArrow.addEventListener("click", () => {
    ...
    let newMonth = currentMonth; /* Dont't scroll past month, only enable future next month */
    let newYear = year;
    let newDate = new Date(newYear, newMonth, 1);
    generateCalendarDays(newDate);
  });

  /* ----- Future month scrolling ----- */
  const rightArrow = document.getElementsByClassName("calendar-right-arrow")[0];
  rightArrow.addEventListener("click", () => {
    const monthYear = document.getElementsByClassName("calendar-month-year")[0];
    const month = parseInt(monthYear.getAttribute("data-month") || 0);
    const year = parseInt(monthYear.getAttribute("data-year") || 0);
    let newMonth = currentMonth + 1; /* enable next month only scroll */
...
function generateCalendarDays(currentDate) {
  ...
  var servMonth = court_date.getMonth();
  var servYear = court_date.getFullYear();
  ...
  while (day <= totalDaysInMonth) {
    let li = document.createElement("li");
    li.textContent = day;
    var weeks = new Date(year, month, day).getDay();
    /* ----- default class value set ----- */
    li.classList.add("calendar-day");
    li.classList.add("cal-disabled");
    if (servDate == day && servMonth == month && servYear == year) {
      li.classList.add("calendar-day-active");
      li.classList.remove("cal-disabled");
    }

    var setmonth = (month + 1);
    var dayformat = (day < 10) ? '0'+day : day;
    var monthformat = (setmonth < 10) ? '0'+setmonth : setmonth;
    let formattedDate = year+'-'+monthformat+'-'+dayformat;
    li.setAttribute('data-date', formattedDate);

    if(currentMonth <= month && currentYear <= year) {
      li.classList.add("cal-date");
      li.classList.remove("cal-disabled");
    }
    
    if(currentMonth < month) {
      let todayDate = new Date(currentYear, currentMonth, todaysDate);
      let RestrictDate = new Date(todayDate.setDate(todayDate.getDate() + 29));
      if(day > RestrictDate.getDate() && currentMonth < month && currentYear <= year) {
        li.classList.add("cal-disabled");
        li.classList.remove("cal-date");
      }
    }

    /* ----- disable weeks ----- */
    if( (jQuery.inArray(weekArray[weeks], settingweeks) < 0) ) {
      li.classList.remove("cal-date");
      li.classList.add("cal-disabled");
    }
    calendarDays.appendChild(li);
