...
class Backend extends CI_Controller {
	...
	public function index() {
		...
		$mon_appoi_cnt = $this->Common_model->RawSQL(" SELECT COUNT(DISTINCT `fld_appointid`) AS mon_appoint_cnt FROM `appointments` WHERE DATE_FORMAT(`fld_adate`, '%Y-%m') = '$year_month'  AND `fld_astatus` != 'Cancelled' AND `fld_atype` IS NULL");

		/* Get today appointment count */
     	$today_appoi_cnt = $this->Common_model->RawSQL(" SELECT COUNT(DISTINCT `fld_appointid`) AS today_appoint_cnt FROM `appointments` WHERE `fld_adate` = '$today' AND `fld_astatus` != 'Cancelled' AND `fld_atype` IS NULL ");
		...
		$today_sales = $this->Common_model->RawSQL(" SELECT SUM(`fld_prate`) AS `today_paid` FROM `payments` WHERE `fld_pdate` BETWEEN '".$today." 00:00:00' AND '".$today." 23:59:59' ");
      
		/* get this monthly revenue value */
		$monthly_sales = $this->Common_model->RawSQL(" SELECT SUM(`fld_prate`) as `monthly_paid` FROM `payments` WHERE DATE_FORMAT(`fld_pdate`, '%Y-%m') = '".$year_month."' ");
